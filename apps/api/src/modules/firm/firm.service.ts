...
export class FirmService {
  ...
  async inviteFirmMember(userId: string, dto: InviteFirmMemberDto) {
    ...
    if (!target) {
      throw new NotFoundException('Firm member must register first with this email.');
    }
    ...
    if (count >= membership.organization.firmUserLimit) {
      throw new BadRequestException(
        `Firm user limit reached (${count}/${membership.organization.firmUserLimit}).`,
      );
    }
  ...
  async inviteClientUser(userId: string, businessId: string, dto: InviteClientUserDto) {
    return this.businesses.inviteClientUser(
      userId,
      businessId,
      dto.email,
      dto.role || 'CLIENT_OWNER',
    );
  }
  ...
  async createAccountTemplate(userId: string, dto: CreateAccountTemplateDto) {
    ...
    return {
      message: 'Firm account library updated',
      template,
    };
  }

  async importDefaultAccounts(userId: string, businessId: string) {
    const { business, firmMembership } = await this.businesses.getUserAccessForBusiness(
      userId,
      businessId,
    );
    ...
    return {
      message: 'Firm default accounts imported/repaired for client',
  ...
  async approveAccountHeadRequest(
    userId: string,
    requestId: string,
    dto: ApproveAccountHeadRequestDto,
  ) {
    const membership = await this.businesses.ensureFirmUser(userId);
    ...
    if (!request) {
      throw new NotFoundException('Account head request not found');
    }
    ...
    const code =
      ...
      (await this.nextAccountCode(request.businessId, dto.type || request.suggestedType));
    ...
    return {
      message: 'Account head approved and created',
      request: updated,
      account,
    };
  }
  ...
  async rejectAccountHeadRequest(userId: string, requestId: string, dto: RejectRequestDto) {
    ...
    return {
      message: 'Account head request rejected',
      request: updated,
    };
  }
  ...
  async decideReportExportRequest(
    userId: string,
    requestId: string,
    ...
    const request = await this.prisma.reportExportRequest.findFirst({
      where: {
        id: requestId,
        organizationId: membership.organizationId,
      },
    ...
    if (!request) {
      throw new NotFoundException('Report export request not found');
    }
